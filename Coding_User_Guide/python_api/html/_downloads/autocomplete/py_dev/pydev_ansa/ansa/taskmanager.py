...
def AparameterTaskItemLinkToAparameter(task_item_ref: base.Entity, ansa_parameter_ref: base.Entity, expression: str) -> int:
	...
	Parameters
	----------
	task_item_ref : base.Entity
		The task item object.

	ansa_parameter_ref : base.Entity
		APARAM object.
...
def ChangeTaskItem(task_item_ref: base.Entity) -> int:
...
def ClearBreakTaskItem(task_item_ref: base.Entity) -> int:

	"""

	Removes break from Task Item.
...
def GetAllTaskItems() -> list[base.Entity]:
...
def GetRootTaskManager() -> base.Entity:
...
def SetBreakTaskItem(item: base.Entity) -> int:

	"""

	Sets a break on a Task Item.
...
def SetToscaOptResultsDir(task_item_ref: base.Entity, directory_str: str) -> int:

	"""

	Sets the directory of the optimization results to the Task Item (GENERATE_REPORT_FILE or RUN_SMOOTH)
...
def LCPointsFisrtTorsionalWizard(nodes_lc_points: list[base.Entity]) -> list:
	...
	Parameters
	----------
	nodes_lc_points : list[base.Entity], optional
...
def GetPostProcessingItemData(task_item: object) -> dict:

	"""

	Get the data from a task manager Post-Processing item
	...
	See Also
	--------
	ansa.taskmanager.SetPostProcessingItemData
	...
	::
		...
		def main():
		    post_item = taskmanager.GetTaskItemsByName("Post-Processing")
		    dict = taskmanager.GetPostProcessingItemData(post_item[0])
		    print(dict)
