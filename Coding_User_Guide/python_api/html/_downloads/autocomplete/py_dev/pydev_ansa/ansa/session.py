...
def ApplicationInformation(format: str='text') -> str:
	...
	-------
	str
		If format ='text', it returns a string containing the build/runtime information. If format = 'dict', the information is returned as a dictionary object.
...
def DeckName(deck: int) -> str:
	...
	------
	ValueError
		Invalid deck constant as argument.

	See Also
	--------
	ansa.base.CurrentDeck, constants
...
def defbutton(group: str, label: str, tip: str, rgb: tuple) -> None:
...
def BenchmarkFPS(frames: int, ctrl_flag: int, shift_flag: int, times_to_press_benchmark: int) -> Iterable[float]:
...
@typing_extensions.deprecated("Deprecated since version 20.0.0.Use ansa.base.BCSettingsWriteFile instead.")
def SaveSettings() -> int:

	"""
	.. deprecated:: 20.0.0
		Use :py:func:`ansa.base.BCSettingsWriteFile` instead.
	...
	Notes
	-----
	This function is deprecated. Use ansa.guitk.BCSettingsWriteFile()

	See Also
	--------
	ansa.base.BCSettingsWriteFile

	"""

	warnings.warn("Deprecated since version 20.0.0.Use ansa.base.BCSettingsWriteFile instead.", DeprecationWarning)
...
def SaveSettingsAs(filename: str) -> int:
	...
	Notes
	-----
	This function is deprecated. Use ansa.guitk.BCSettingsWriteFile(filename)
...
def GetReadyForNewTask() -> None:
	...
	* If ANSA is running as a worker and script execution was triggered by the 
	...
	-------
	None
		This function always raises an exception / never returns a value

	Raises
	------
	Exception
		An Exception object is always raised by this function
...
def SystemInfo(format: str='text') -> str | dict:
