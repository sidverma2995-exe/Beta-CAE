...
def CalculateMassProperties(entity: base.Entity) -> int:
	...
	------
	TypeError
		Invalid entity given (not RIGBODY).
...
def Dz(to_marker: str, from_marker: str, along_marker: str) -> float:
	...
	Notes
	-----
	If an optional argument is not provided, a marker parallel to the global coordinate system located at 0,0,0 is used.
	Both camelcase and lowercase are valid for this function.
...
def Vr(to_marker: str, from_marker: str, reference_marker: str) -> float:
	...
	-----
	If an optional argument is not provided, a marker parallel to the global coordinate system located at 0,0,0 is used.
	VR is negative when the 'to_marker' and 'from_marker' approach each other.
	VR is positive, as 'to_marker' and 'from_marker' separate from each other.
	Both camelcase and lowercase are valid for this function.
...
def AddBodyToContact(kin_contact: base.Entity, kin_body: base.Entity) -> int:
...
def FindContact(kin_contact: base.Entity) -> tuple:
...
def SaveAsInitialPosition(initialize_joints: bool=False, kin_config: base.Entity=None) -> int:
	...
	kin_config : base.Entity, optional
		A KIN_CONFIG entity.
...
def SavePosition(kin_config: base.Entity, name: str) -> base.Entity:
	...
	Notes
	-----
	NOTE: Pass the KIN_CONFIG as argument after movement of the model from the Configurator (ansa.kinetics.MoveConfig).
...
def MoveToPosition(kin_pos: base.Entity) -> int:
...
def MoveByResults(kin_results: base.Entity, time: float) -> float:
...
def ResultsWriteToXML(kin_result: base.Entity, file_name: str, output_adams_view: bool, adams_name: bool) -> int:
...
def TableCreate(name: str, interpolation: str, items: list[list]) -> base.Entity:
...
def TableSetData(entity: base.Entity, items: list[list]) -> int:
...
def TableGetData(entity: base.Entity) -> list:
...
def ResultsGetData(res_entity: base.Entity, kin_entity: base.Entity) -> dict:
	...
	Parameters
	----------
	res_entity : base.Entity
		A KIN_RESULTS entity.

	kin_entity : base.Entity
...
def MotionDefine(entity: base.Entity | list[base.Entity], position1: list[float] | tuple[float, float, float], position2: list[float] | tuple[float, float, float], direction1: list[float] | tuple[float, float, float], direction2: list[float] | tuple[float, float, float], along_x: str, along_y: str, along_z: str, about_x: str, about_y: str, about_z: str, function_dx: str, function_dy: str, function_dz: str, function_rx: str, function_ry: str, function_rz: str, disp_ic_dx: float, disp_ic_dy: float, disp_ic_dz: float, disp_ic_rx: float, disp_ic_ry: float, disp_ic_rz: float, vel_ic_dx: float, vel_ic_dy: float, vel_ic_dz: float, vel_ic_rx: float, vel_ic_ry: float, vel_ic_rz: float) -> base.Entity | int:
	...
	position1 : list[float] | tuple[float, float, float], optional
		A sequence (list or tuple) of three(3) floats to determine the position
		 of the 1st marker.

	position2 : list[float] | tuple[float, float, float], optional
		A sequence (list or tuple) of three(3) floats to determine the position 
		of the 2nd marker.

	direction1 : list[float] | tuple[float, float, float], optional
		A sequence (list or tuple) of three(3) floats to determine the direction 
		of the 1st marker.

	direction2 : list[float] | tuple[float, float, float], optional
		A sequence (list or tuple) of three(3) floats to determine the direction 
		of the 2nd marker.
...
def AddToRigidBody(body: base.Entity, elements: Iterable[base.Entity], remove_from_other_body: bool=False) -> int:
	...
	Notes
	-----
	This function applies only for bodies of RIG_BODY GEN_TYPE.

	See Also
	--------
	ansa.kinetics.RemoveFromRigidBody
...
def RemoveFromRigidBody(body: base.Entity, elements: base.Entity | list[base.Entity]) -> int:
	...
	elements : base.Entity | list[base.Entity]
...
def ConvertToTransformation(CONFIG: base.Entity, POSITION: str) -> int:
...
def SetJointStatus(JOINT: base.Entity, STATUS: str) -> str:
...
def AddConfigToConfig(PARENT: base.Entity, CHILD: base.Entity) -> int:
...
def AddJointToConfig(CONFIG: base.Entity, JOINT: base.Entity) -> int:
...
def MoveConfig() -> list | int:
	...
	See Also
	--------
	ansa.kinetics.ConfigSetParameters
...
def ConfigSetParameters(kin_config: int, move_method: str, kin_joint: int, displacement: float, angle: float, translate_x: float, translate_y: float, translate_z: float, rotate_x: float, rotate_y: float, rotate_z: float, sp: int | base.Entity, tp: int | base.Entity, motor_joint: int, end_time: float, start_at_equilibrium: bool, step_size_type: str, step_size: float, number_of_steps: int, results_file: str, max_step_size: float, translate_x_total: float, translate_y_total: float, translate_z_total: float, rotate_x_total: float, rotate_y_total: float, rotate_z_total: float, displacement_total: float, angle_total: float, lock_translate_x: bool=False, lock_translate_y: bool=False, lock_translate_z: bool=False, lock_rotate_x: bool=False, lock_rotate_y: bool=False, lock_rotate_z: bool=False, matching_points_by_actuator_joint: bool=False, sp_body: base.Entity=None) -> int:
	...
	------
	TypeError
		KinematicsSetParameters: More than one direction movement is given for method 'BY JOINT' or 'BY ACTUATOR JOINT'
...
def MotionGetData(entity: base.Entity) -> dict:
...
def RemoveBodyFromContact(kin_contact: base.Entity, kin_body: base.Entity) -> int:
...
def GetBodiesFromContact(kin_contact: base.Entity) -> list[base.Entity]:
...
def AddExcludedToContact(kin_contact: base.Entity, elements: list[base.Entity]) -> int:
...
def ClearContact(kin_contact: base.Entity, only_excluded: bool=False) -> int:
...
def ContactBodySetData(kin_contact: base.Entity, kin_body: base.Entity, shape: str, thickness: float, thickness_scale: float, smooth_subdiv_lev: int, smooth_feat_ang: float) -> int:
...
def GetModelInfo(kin_config: base.Entity=None) -> dict:
	...
	dict
		This function returns a dictionary with keywords:
		    ...
		    "markers_on_curve"          : number of Markers on Curves
		    "equations"                 : number of DIFF Equations
...
def GetJointsFromConfig(kin_config: base.Entity) -> base.Entity:
...
def RemoveJointFromConfig(kin_config: base.Entity, kin_joint: base.Entity) -> int:
...
def ConfigsJointSetStatus(kin_config: base.Entity, kin_joint: base.Entity, locked: bool) -> int:
...
def ConfigsJointIsLocked(kin_config: base.Entity, kin_joint: base.Entity) -> bool:
...
def AddForceToConfig(CONFIG: base.Entity, FORCE: base.Entity) -> int:
...
def ConvertPosition(position: base.Entity, conversion_type: str, output_directory: str, locations_type: str, velocities_type: str, sets_type: str, use_position_name: bool, position_prefix_name: str, include_transform_file_name: str, include_trasform_full_path_names: bool, morph_nodes_file_name: str, output_kinetics: bool, deformed_bodies: list[str], include_transform_enable_file: bool, morph_nodes_enable_file: bool, output_parameters: bool, parameters_file_name: str, reference_position: base.Entity, output_foam_ref_geom: bool, create_folders: bool) -> int:
	...
	deformed_bodies : list[str], optional
	...
	reference_position : base.Entity, optional
...
def Yaw(to_marker: str, from_marker: str) -> float:
	...
	Notes
	-----
	If the optional argument "from_marker" is not provided, a marker parallel to the 
	global coordinate system located at (0,0,0) is used.
...
def Psi(to_marker: str, from_marker: str) -> float:
	...
	-----
	If an optional argument is not provided, a marker parallel to the global coordinate system located at 0,0,0 is used. 
	Although the rotation sequence is Phi-The-Psi, this function returns the first angle (instead of the third) for compatibility with MSC Adams.
...
def Phi(to_marker: str, from_marker: str) -> float:
	...
	-----
	If an optional argument is not provided, a marker parallel to the global coordinate system located at 0,0,0 is used.
	Although the rotation sequence is Phi-The-Psi, this function returns the third angle (instead of the first) for compatibility with MSC Adams.
...
def Joint(joint: str, computed_at_marker: str, component: str, ref_marker: str) -> float:
	...
	Notes
	-----
	In order to return the results in the global coordinate system, set 'ref_marker' = 0.
...
def Akispl(x: float, z: float, kin_table: str, deriv_order: str) -> float:
	...
	Notes
	-----
	For a two dimensional 'kin_table', set z = 0.
	For a three dimensional 'kin_table', the derivation should not be used ('deriv_order' = 0).
...
def GetArticulationHistoryFromConfig(kin_config: base.Entity) -> list:
...
def GetMarkers(kin_entity: base.Entity) -> list[base.Entity]:
...
def HBMConfigMove(config: base.Entity, total_angle: float, step: float, pelvis_lumbar_ratio: float, pelvic_stop_angle: float) -> bool:
	...
	step : float, optional
		The incremental step for applying the total angle. Default value is 5.0 degrees.

	pelvis_lumbar_ratio : float, optional
		Define the Pelvis to Lumbar spine tilt ratio during pelvic_lumbar_extension or pelvic_lumbar_flexion movements.

	pelvic_stop_angle : float, optional
		Define the Pelvis stop angle during pelvic_lumbar_extension or pelvic_lumbar_flexion movements. This argument is exclusive to the pelvis_lumbar_ratio.
...
def HBMTreeJointRotate(joint: base.Entity, total_angle: float, axis: int, step: float=1.0) -> bool:
...
def AutoMech(starting_element: base.Entity, joints: Iterable[base.Entity], elements_to_disregard: Iterable[base.Entity], create_dummy_kin_model: bool=False, create_sets_with_properties: bool=False, create_kinetic_forces: bool=False, group_name: str='Mechanism from AutoMech') -> list[base.Entity]:
	...
	elements_to_disregard : Iterable[base.Entity], optional
...
def PositionGetBodiesJoints(kin_pos: base.Entity) -> list[base.Entity]:
...
def MoveConfigInteractive(kin_config: base.Entity) -> int:
	...
	Notes
	-----
	The function works only on gui mode.
...
class Simulator():
	...
	eigenvalue_analysis: bool = None
	"""
	Eigenvalue analysis.
	(If true, run eigenvalue analysis)
	It can be activated for Initial Condition, Equilibrium and Dynamic analysis.
	...
	def __init__(self, kin_config: object, type: str, end_time: float, find_equilibrium_first: bool, animate_during_calc: bool) -> None:
...
class FlexBody():
	...
	def __init__(self, filename: str, body: object, delete_body_contents: bool, align_with_body_cog: bool) -> None:
...
def HBMReadPosFile(file: str, step: float=5.0, hbm_random_element: base.Entity=None) -> int:
	...
	hbm_random_element : base.Entity, optional
	...
	See Also
	--------
	ansa.kinetics.HBMConfigMove, ansa.kinetics.HBMTreeJointRotate
...
def Contact(contact: str, computed_on_body: str, component: str, ref_marker: str) -> float:

	"""

	Returns the component of the impact force of a 'contact' as calculated in the coordinate system of marker 'ref_marker'.

	Parameters
	----------
	contact : str
		Id of the 'contact' under consideration

	computed_on_body : str
		The body on which the contact force/torque will be calculated (body that participates in the contact definition).
		If 0: the body with the lowest Id.
		If 1: the body with the highest Id.

	component : str
		The component of the contact force,
		If 1: magnitude of the total contact force (FM)
		If 2: x component of the total contact force (FX)
		If 3: y component of the total contact force (FY)
		If 4: z component of the total contact force (FZ)
		If 5: magnitude of the total contact torque (TM)
		If 6: x component of the total contact torque (TX)
		If 7: y component of the total contact torque (TY)
		If 8: z component of the total contact torque (TZ)

	ref_marker : str
		Id of the reference marker used for the calculation of the contact force/torque.

	Returns
	-------
	float
		Returns a component of the impact force of a 'contact' (scalar).

	Notes
	-----
	In order to get the results in the global coordinate system, set 'ref_marker'='0'.
	The function should be used if the contact entity contains only two bodies.
	...
	::
		...
		def main():
		    contact_force = kinetics.Contact("1", "1", "2", "5")
