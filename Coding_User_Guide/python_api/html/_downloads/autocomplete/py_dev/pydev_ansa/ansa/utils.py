...
def CreateCurve() -> object:
	...
	See Also
	--------
	ansa.utils.AddCurveData, ansa.utils.DeleteCurve, ansa.utils.AddCurveToPlot
...
def DeckInfo(filename: str, mode: str, text_format: str, active_options: list[str]=[], threshold_values: dict[str, str]={}, parameter_values: dict[str, str]={}, f11_sync: dict[str, str]={}) -> int:
	...
	text_format : str
		Accepted values: 'HTML', 'TEXT', 'CSV', 'PDF' or 'JSON'.

	active_options : list[str], optional
	...
	threshold_values : dict[str, str], optional
		A python dictionary.
	...
	parameter_values : dict[str, str], optional
		A python dictionary.
	...
	f11_sync : dict[str, str], optional
		A python dictionary.
	...
	The PDF text_format requires running ANSA with a graphical user interface
	...
	::
		...
		def main():
		    ...
		    # will create two json files: masses.json and masses.json.schema
		    # the first one contains the requested results
		    # the second one is a supplementary file for documentation purposes,
		    # it is a JSON schema for defining the structure (key/value_type) of JSON data
		    # in the main output file
		    utils = utils.DeckInfo(
		        "/home/users/masses.json", "WHOLE_DB", "JSON", active_options=["mass_info"]
		    )
...
def GetParametersFromAnsaDB(ansa_file_name: str) -> dict:
...
@typing_extensions.deprecated("Deprecated since version 17.0.0.Use ansa.utils.OpenModelBrowser instead.")
def OpenPartManager(view_mode: str) -> int:

	"""
	.. deprecated:: 17.0.0
		Use :py:func:`ansa.utils.OpenModelBrowser` instead.
	...
	Notes
	-----
	Deprecated function in v17.0. Replaced by OpenModelBrowser.
	...
	warnings.warn("Deprecated since version 17.0.0.Use ansa.utils.OpenModelBrowser instead.", DeprecationWarning)

@typing_extensions.deprecated("Deprecated since version 18.0.0.Use ansa.utils.SnapShot instead.")
def PrintToFile(filename: str, image_format: str, red: int, green: int, blue: int, transparent: bool, text_axes: bool, gradient: bool) -> int:
	...
	"""
	.. deprecated:: 18.0.0
		Use :py:func:`ansa.utils.SnapShot` instead.
	...
	warnings.warn("Deprecated since version 18.0.0.Use ansa.utils.SnapShot instead.", DeprecationWarning)
...
def SnapShot(filename: str, image_format: str, red: int, green: int, blue: int, transparent: bool, text_axes: bool, gradient: bool, foreground_red: int, foreground_green: int, foreground_blue: int, auto_text_color: bool, image_size: tuple[int, int], copy_to_clipboard: bool) -> int:
	...
	Notes
	-----
	This function does not work when ANSA is launched using the -nogui argument.
	The --batch-mode argument can be used instead.

	See Also
	--------
	ansa.utils.SnapShotWidget, ansa.utils.SnapShotWindow
...
@typing_extensions.deprecated("Deprecated since version 24.1.0.Use ansa.utils.ModelBrowser instead.")
def OpenModelBrowser(entity_type: str, view_mode: str, main_tabs: Iterable[str], current_main_tab: str, info_tabs: Iterable[str], current_info_tab: str, side_panel: bool, current_side_panel_tab: str, profile: str) -> bool:

	"""
	.. deprecated:: 24.1.0
		Use :py:class:`ansa.utils.ModelBrowser` instead.
	...
	main_tabs : Iterable[str], optional
	...
	info_tabs : Iterable[str], optional
	...
	Notes
	-----
	If a profile name is defined all other arguments are ignored.
	Deprecated function in v24.1.0. Replaced by ModelBrowser class.

	See Also
	--------
	ansa.utils.ModelBrowser, ansa.utils.ModelBrowserTab, ansa.utils.CloseModelBrowser, ansa.utils.ShowModelBrowserColumn, ansa.utils.HideModelBrowserColumn
	...
	warnings.warn("Deprecated since version 24.1.0.Use ansa.utils.ModelBrowser instead.", DeprecationWarning)

@typing_extensions.deprecated("Deprecated since version 24.1.0.Use ansa.utils.ModelBrowser instead.")
def CloseModelBrowser() -> None:
	...
	Notes
	-----
	Deprecated function in v24.1.0. Replaced by ModelBrowser.

	See Also
	--------
	ansa.utils.ModelBrowser, ansa.utils.ModelBrowserTab, ansa.utils.OpenModelBrowser, ansa.utils.ShowModelBrowserColumn, ansa.utils.HideModelBrowserColumn
...
def DatabaseBrowserInfo(deck: int, extra_info: list, simplify: str='default') -> dict:
...
def ReadGUISettings(xml_file_path: str) -> bool:
	...
	See Also
	--------
	ansa.utils.SaveGUISettings, ansa.utils.CurrentGUISettings
...
def CurrentGUISettings() -> str:
	...
	See Also
	--------
	ansa.utils.ReadGUISettings, ansa.utils.SaveGUISettings

	"""

def SnapShotWidget(filename: str, image_format: str, widget: guitk.BCGuiObject) -> int:
	...
	widget : guitk.BCGuiObject, optional
	...
	Notes
	-----
	This function does not work in nogui mode.

	See Also
	--------
	ansa.utils.SnapShot, ansa.utils.SnapShotWindow
...
def SnapShotWindow(filename: str, image_format: str, window_title: str) -> int:
	...
	See Also
	--------
	ansa.utils.SnapShot, ansa.utils.SnapShotWidget
...
@typing_extensions.deprecated("Deprecated since version 24.1.0.Use ansa.utils.ModelBrowserTab instead.")
def ShowModelBrowserColumn(column_name: str, position: int) -> None:

	"""
	.. deprecated:: 24.1.0
		Use :py:class:`ansa.utils.ModelBrowserTab` instead.
	...
	Notes
	-----
	Deprecated function in v24.1.0. Replaced by ModelBrowserTab.

	See Also
	--------
	ansa.utils.ModelBrowser, ansa.utils.ModelBrowserTab, ansa.utils.HideModelBrowserColumn, ansa.utils.OpenModelBrowser, ansa.utils.CloseModelBrowser
	...
	warnings.warn("Deprecated since version 24.1.0.Use ansa.utils.ModelBrowserTab instead.", DeprecationWarning)

@typing_extensions.deprecated("Deprecated since version 24.1.0.Use ansa.utils.ModelBrowserTab instead.")
def HideModelBrowserColumn(column_name: str) -> None:
	...
	See Also
	--------
	ansa.utils.ModelBrowser, ansa.utils.ModelBrowserTab, ansa.utils.ShowModelBrowserColumn, ansa.utils.OpenModelBrowser, ansa.utils.CloseModelBrowser
...
def SetDrawingForegroundColor(r: int, g: int, b: int) -> None:
	...
	See Also
	--------
	ansa.utils.DrawingForegroundColor, ansa.utils.SetDrawingAreaColor
...
def DrawingForegroundColor() -> tuple[float]:
	...
	See Also
	--------
	ansa.utils.SetDrawingForegroundColor
...
def AddCurveToPlot(plot: object, curve: object) -> None:
	...
	See Also
	--------
	ansa.base.CreateCurve, ansa.utils.DeleteCurve, ansa.utils.CreatePlot, ansa.utils.DeletePlot
...
def CreatePlot() -> object:
	...
	See Also
	--------
	ansa.utils.SetPlotProperties, ansa.utils.DeletePlot
...
def RecentFilesRecordingSetEnabled(enabled: bool) -> None:
	...
	See Also
	--------
	ansa.utils.RecentFilesRecordingIsEnabled
...
def RecentFilesRecordingIsEnabled() -> bool:
	...
	See Also
	--------
	ansa.utils.RecentFilesRecordingSetEnabled
...
def FringeBarUnpinPosition(type: str="GENERAL") -> None:
	...
	See Also
	--------
	ansa.utils.FringeBarPinPosition
...
def FringeBarPinPosition(position: int, type: str, coordinates: list[float]) -> None:
	...
	See Also
	--------
	ansa.utils.FringeBarUnpinPosition
...
def DetectThermalLinks(dist: float, entities: list[base.Entity] | str="all", excl_angle: int=3, self_link: bool=False) -> list:
	...
	entities : list[base.Entity] | str, optional
...
class UnitSystem():
	...
	def __init__(self, length: str='milimeter', mass: str='tonne', time: str='second', temperature: str='kelvin', kinetics_force: str='Newton') -> None:
		...
		kinetics_force : str, optional
			The force unit for kinetics (Default: "Newton")
	...
	@staticmethod
	def set_current(unit_system: UnitSystem, apply_to_settings: bool=True, apply_to_models: bool=False) -> None:
...
def XlsxGetCellValue(file: object, sheet: object, row: int, column: int) -> str:
	...
	See Also
	--------
	ansa.utils.XlsxOpen, ansa.utils.XlsxCreate, ansa.utils.XlsxInsertSheet, ansa.utils.XlsxGetCellValueByName, ansa.utils.XlsxSetCellValue, ansa.utils.XlsxSetCellValueByName, ansa.utils.XlsxSave, ansa.utils.XlsxSheetsCount, ansa.utils.XlsxMaxSheetCell, ansa.utils.XlsxClose
...
def XlsxSetCellValue(file: object, sheet: object, row: int, column: int, value: object) -> int:
	...
	See Also
	--------
	ansa.utils.XlsxOpen, ansa.utils.XlsxCreate, ansa.utils.XlsxInsertSheet, ansa.utils.XlsxGetCellValue, ansa.utils.XlsxGetCellValueByName, ansa.utils.XlsxSetCellValueByName, ansa.utils.XlsxSave, ansa.utils.XlsxSheetsCount, ansa.utils.XlsxMaxSheetCell, ansa.utils.XlsxClose
...
def AddCurveData(curve: object, coords: object) -> None:
	...
	See Also
	--------
	ansa.base.CreateCurve, ansa.utils.DeleteCurve
...
def GetGlobalMountMappingTable() -> list[MountMapping]:
	...
	See Also
	--------
	ansa.utils.MountMapping, ansa.utils.SetGlobalMountMappingTable
...
def SetGlobalMountMappingTable(mount_mapping_table: list[MountMapping]) -> None:
	...
	See Also
	--------
	ansa.utils.MountMapping, ansa.utils.GetGlobalMountMappingTable
...
def LaunchLingeringMLWorker(context_dir: str, cmd: list[str], env: str='consolidated', dbg: bool=False) -> bool:
	...
	See Also
	--------
	ansa.utils.LingeringMLInstance, ansa.utils.LingeringMLClient
...
class LingeringMLClient():
	...
	See Also
	--------
	ansa.utils.LingeringMLInstance, ansa.utils.BALStreamsCommunicator
...
def RunParallelJobsInBackground(script_paths: list, command: str, max_num_workers: int, script_function_to_call: str, redirection_logs_path: str, job_time_limit: int, worker_launch_time_limit: int, worker_initialization_script: str, jobs_group_name: str, job_names: list, delete_scripts_when_done: bool=False) -> None:
	...
	See Also
	--------
	ansa.utils.ParallelJobsDispatcher
...
class ModelBrowser():
	...
	A class that handles all the Model Browser functionality.

	See Also
	--------
	ansa.utils.ModelBrowserTab
	...
	::
		...
		def main():
		    # Open of Model Browser
		    mb = utils.ModelBrowser()
		    mb.show()
		
		    # Open second Model Browser window
		    mb_2nd = mb.open_new_window()
		
		    # Set name to the second Model Browser window
		    mb_2nd.set_name("Second Model Browser")
		
		    # Open 3 new tabs in the second Model Browser window
		    tab1 = mb_2nd.open_tab("Parts", True)
		    tab1.set_name("Parts 1")
		    tab2 = mb_2nd.open_tab("Parts", True)
		    tab2.set_name("Parts 2")
		    tab3 = mb_2nd.open_tab("Parts", True)
		    tab3.set_name("Parts 3")
		
		    # Open 2 info tabs in the second Model Browser window
		    mb_2nd.open_info_tab("Contents")
		    mb_2nd.open_info_tab("Details")
		
		    # Set the info tab "Contents" as current info tab
		    mb_2nd.set_current_info_tab("Contents")
		
		    # Close the second Model Browser window
		    mb_2nd.close()
		
		    # Close the first Model Browser window
		    mb.close()
	...
	def show(self) -> bool:
		...
		Shows the Model Browser window.
	...
	def hide(self) -> bool:
		...
		Hides the Model Browser window.
	...
	def close(self) -> bool:
		...
		Closes the Model Browser window. In case of main model Model Browser the function hides the window instead of closing it.
	...
	def is_window_shown(self) -> bool:
		...
		Checks if the Model Browser window is shown.


		Returns
		-------
		bool
			Returns True in case of Model Browser window is shown and False otherwise.
		...
		::
			...
			def main():
			    ...
			    is_shown = mb.is_window_shown()
			    print(f"Model Browser Window is shown: {is_shown}")
	...
	def open_new_window(self) -> ModelBrowser:

		"""

		Creates a secondary Model Browser window.
		...
		::
			...
			def main():
			    ...
			    # Set name to the second window
			    mb_2nd.set_name("Second Model Browser")
			
			    # Get name from the second window
			    name = mb_2nd.get_name()
			    print(f"Name of the second window: {name}")
	...
	def set_name(self, name: str) -> bool:
		...
		Sets the given name to secondary window. In case of main window the function fails.


		Parameters
		----------
		name : str
			The name that will be given to the window.

		Returns
		-------
		bool
			Returns True in case of successful name set, False otherwise.
	...
	def get_name(self) -> str:
		...
		Returns the name of the Model Browser window.
	...
	def open_tab(self, name: str, new_tab: bool=False) -> ModelBrowserTab:

		"""

		Opens the tab that has the given name.
		Note that this function will not make the tab current, use set_current_tab instead.


		Parameters
		----------
		name : str
			The name of the tab that will be opened.
			Accepted values:
			- The tab names as they are shown: 'Parts', 'Configurations', 'Subsystems', etc.
			- The ANSA keywords: 'ANSAPART', 'ANSA_CONFIGURATION', 'ANSA_SUBSYSTEM', etc.

		new_tab : bool, optional
			Flag that asks the opening of a new tab even if the tab with the specific name is already open.

		Returns
		-------
		ModelBrowserTab
			Returns the opened tab, None otherwise.
		...
		::
			...
			def main():
			    ...
			    # Open tabs
			    tab_parts = mb.open_tab("Parts")
			    tab_parts2 = mb.open_tab("ANSAPART", True)
			    tab_configs = mb.open_tab("Configurations", False)
			
			    # Close tab
			    mb.close_tab("Configurations")
			
			    # Check if the tab is open
			    is_open = mb.is_tab_open("Configurations")
			    print(f"The Configurations tab is open: {is_open}")
	...
	def close_tab(self, name: str) -> bool:
		...
		Closes the tab of the given name. Note that if the tab is the last open tab, it will not be closed.


		Parameters
		----------
		name : str
			The name of the tab that will be closed.
			Accepted values:
			- The tab names as they are shown: 'Parts', 'Configurations', 'Subsystems', etc.
			- The ANSA keywords: 'ANSAPART', 'ANSA_CONFIGURATION', 'ANSA_SUBSYSTEM', etc.
		...
		-------
		bool
			Returns True in case of successful closing, False otherwise.
	...
	def is_tab_open(self, name: str) -> bool:

		"""

		Checks if the tab with the given name is open.


		Parameters
		----------
		name : str
			The name of the tab that will be checked.
			Accepted values:
			- The tab names as they are shown: 'Parts', 'Configurations', 'Subsystems', etc.
			- The ANSA keywords: 'ANSAPART', 'ANSA_CONFIGURATION', 'ANSA_SUBSYSTEM', etc.
	...
	def get_tab(self, name: str) -> ModelBrowserTab:

		"""

		Returns the tab with the specific name.
		...
		::
			...
			def main():
			    ...
			    # Get tabs
			    tab1 = mb.get_tab("Parts")
			    name = tab1.get_name()
			    print(name)
			
			    tabs = mb.get_open_tabs()
			    for tab in tabs:
			        name = tab.get_name()
			        print(name)
			
			    # Set current tab
			    mb.set_current_tab("Configurations")
			
			    # Get current tab
			    tab = mb.get_current_tab()
			    name = tab.get_name()
			    print(name)
	...
	def get_open_tabs(self) -> list:
		...
		Returns a list of the open tabs.


		Returns
		-------
		list
			The list contains ModelBrowserTab objects.
	...
	def set_current_tab(self, name: str) -> bool:
		...
		----------
		name : str
			The name of the tab that will be set as current.
			Accepted values:
			- The tab names as they are shown: 'Parts', 'Configurations', 'Subsystems', etc.
			- The ANSA keywords: 'ANSAPART', 'ANSA_CONFIGURATION', 'ANSA_SUBSYSTEM', etc.
		...
		-------
		bool
			Returns True in case of successful current set, False otherwise.
	...
	def get_current_tab(self) -> ModelBrowserTab:
	...
	def open_info_tab(self, name: str) -> bool:

		"""

		Opens the info tab with the specific name.


		Parameters
		----------
		name : str
			The name of the info tab that will be opened.
			Accepted values:
			- The tab names as they are shown: 'Details', 'Contents', 'Connectivity', etc.
		...
		::
			...
			def main():
			    ...
			    # Open info tab
			    mb.open_info_tab("Details")
			
			    # Close info tab
			    mb.close_info_tab("Details")
			
			    # Check if info tab is open
			    is_open = mb.is_info_tab_open("Details")
			    print(f"The Details info tab is open: {is_open}")
			
			    # Set current info tab
			    mb.set_current_info_tab("Contents")
	...
	def close_info_tab(self, name: str) -> bool:

		"""

		Closes the info tab with the specific name. Note that if the the tab is the last open info tab, it will not be closed.


		Parameters
		----------
		name : str
			The name of the info tab that will be closed.
			Accepted values:
			- The tab names as they are shown: 'Details', 'Contents', 'Connectivity', etc.
	...
	def is_info_tab_open(self, name: str) -> bool:

		"""

		Checks if the info tab with the specific name is open.


		Parameters
		----------
		name : str
			The name of the info tab.
			Accepted values:
			- The tab names as they are shown: 'Details', 'Contents', 'Connectivity', etc.
		...
		-------
		bool
			Returns True if the info tab is open, False otherwise.
	...
	def set_current_info_tab(self, name: str) -> bool:

		"""

		Sets the info tab as current.


		Parameters
		----------
		name : str
			The name of the info tab that will be current.
			Accepted values:
			- The tab names as they are shown: 'Details', 'Contents', 'Connectivity', etc.
		...
		-------
		bool
			Returns True in case of successful set of info tab as current, False otherwise.
	...
	def open_side_panel(self) -> bool:
		...
		::
			...
			def main():
			    ...
			    is_open = mb.is_side_panel_open()
			    print(f"Side panel is open: {is_open}")
	...
	def close_side_panel(self) -> bool:
	...
	def is_side_panel_open(self) -> bool:
		...
		Checks if the side panel is open.


		Returns
		-------
		bool
			Returns True if the side panel is open, False otherwise.
	...
	def set_profile(self, name: str) -> bool:
		...
		Sets the given profile to the Model Browser.


		Parameters
		----------
		name : str
			The name of the profile that will be set.
			Accepted values:
			- Any of the Profile names which exist in the Model Browser Profile settings.
		...
		-------
		bool
			Returns True in case of successful profile set, False otherwise.
	...
	def get_profile(self) -> str:
		...
		Returns the name of the current profile that is applied in Model Browser.
...
class ModelBrowserTab():

	"""

	A class that handles all the Model Browser main tab functionality. Note that objects of this class can be taken only via ModelBrowser methods that return ModelBrowserTab (e.g. ModelBrowser.get_tab()).

	See Also
	--------
	ansa.utils.ModelBrowser
	...
	::
		...
		def main():
		    ...
		    # Open the tab "Parts"
		    tab = mb.open_tab("Parts")
		
		    # Change the name of the tab
		    tab.set_name("My Parts")
		    ...
		    tab.set_visible_columns(visible_columns)
		
		    # Change the view
		    tab.set_view("Flat View")
	...
	def close(self) -> bool:
		...
		Closes the current tab. Note that if the current tab is the last open tab, it will not be closed.
	...
	def set_current(self) -> bool:
		...
		::
			...
			def main():
			    ...
			    # Check if the tab is current
			    is_current = tab.is_current()
			    print(f"Tab is current: {is_current}")
	...
	def is_current(self) -> bool:
		...
		Checks if the tab is current.


		Returns
		-------
		bool
			Returns True if the tab is current, False otherwise.
	...
	def show_column(self, column_name: str, position: int=-1) -> bool:

		"""

		Shows the column with the specific column_name in the tab. The column is shown in the given position. The position count starts from 0. In case of position 0 the column appears first while in case of position -1 the column appears last.


		Parameters
		----------
		column_name : str
			The name of the column that will be shown.

		position : int, optional
			The position of the column show. In case of value -1 the column appears last.

		Returns
		-------
		bool
			Returns True in case of successful column show, False otherwise.
		...
		::
			...
			def main():
			    ...
			    # Show column
			    tab.show_column("Type", 0)
			
			    # Hide column
			    tab.hide_column("Type")
	...
	def hide_column(self, column_name: str) -> bool:

		"""

		Hides the column with the specific column_name from the tab. Note that if the column is the last shown column in the the tab, it will not be hidden.


		Parameters
		----------
		column_name : str
			The name of the column that will be hidden.

		Returns
		-------
		bool
			Returns True in case of successful column hiding, False otherwise.
	...
	def get_visible_columns(self) -> list:
		...
		Returns a list with all visible columns of the tab.
		...
		::
			...
			def main():
			    ...
			    # Get visible columns
			    visible_columns = tab.get_visible_columns()
	...
	def set_visible_columns(self, columns: list) -> bool:

		"""

		Sets the visible columns of the tab. It also hides any other column that is already open. Note that the columns will be set visible in the same order as they are given in the list argument.


		Parameters
		----------
		columns : list
			The list of the columns that will be set visible.

		Returns
		-------
		bool
			Returns True in case of successful opening of columns, False otherwise.
	...
	def get_window(self) -> ModelBrowser:

		"""

		Returns the Model Browser window in which the tab exists.
		...
		::
			...
			def main():
			    ...
			    # Get window
			    window = tab.get_window()
			    name = window.get_name()
			    print(f"Window name is: {name}")
	...
	def clear_selected(self) -> bool:
		...
		Clears the selected entities of the tab.
		...
		::
			...
			def main():
			    ...
			    entities = base.CollectEntities(constants.NASTRAN, None, "ANSAPART")
			    tab.set_selected_entities(entities)
			
			    # Get selected entities
			    selected_entities = tab.get_selected_entities()
	...
	def get_selected_entities(self) -> list:
		...
		Returns a list of all selected entities of the tab.
	...
	def set_selected_entities(self, entities: list) -> bool:

		"""

		Sets the entities of the tab that will be selected.


		Parameters
		----------
		entities : list
			The list of entities that will be selected.

		Returns
		-------
		bool
			Returns True in case of successful entities set, False otherwise.
	...
	def show_specific_entities(self, entities: list) -> bool:

		"""

		Shows only specific entities in the tab (isolated tab) instead of all entities.


		Parameters
		----------
		entities : list
			The list of entities that will be shown in the tab.

		Returns
		-------
		bool
			Returns True in case of successful show of the entities, False otherwise.
		...
		::
			...
			def main():
			    ...
			    # Open the tab "Parts"
			    tab = mb.open_tab("Parts", True)
			    tab.set_current()
			
			    # Show specific entities in tab
			    entities = base.CollectEntities(constants.NASTRAN, None, "ANSAPART")
			    selected_entities = [entities[0], entities[1], entities[2]]
			    tab.show_specific_entities(selected_entities)
			
			    entities_got = tab.get_shown_entities()
			    print(f"Shown entities: {len(entities_got)}")
	...
	def show_all_entities(self) -> bool:

		"""

		Shows all the existing entities in the tab. This functionality is meaningful in case of only specific entities were shown in the tab before.


		Returns
		-------
		bool
			Returns True in case of successful show of entities, False otherwise.
	...
	def get_shown_entities(self) -> list:

		"""

		Returns a list with the shown entities of the tab.
	...
	def set_view(self, name: str) -> bool:
		...
		Sets the view mode of the tab to the given one.
		...
		::
			...
			def main():
			    ...
			    # Get view
			    view = tab.get_view()
			    print(f"The view mode of the tab is: {view}")
	...
	def get_view(self) -> str:
		...
		Returns the current view mode of the tab.
	...
	def set_name(self, name: str) -> bool:
		...
		Sets the name of the tab to the given one.
		...
		::
			...
			def main():
			    ...
			    # Get name
			    name = tab.get_name()
			    print(f"Name of the tab is: {name}")
	...
	def set_position(self, position: int) -> bool:
		...
		Sets the tab in specific position in the tab widget.


		Parameters
		----------
		position : int
			The position index of the tab. The position count starts from 0. Index 0 sets the tab in the first position while index -1 sets the tab in the last position.

		Returns
		-------
		bool
			Returns True in case of successful placing of the tab, False otherwise.
		...
		::
			...
			def main():
			    ...
			    # Get tab position
			    position = tab.get_position()
			    print(f"Position index of the tab is: {position}")
	...
	def get_position(self) -> int:
		...
		Returns the position index of the tab.


		Returns
		-------
		int
			Returns the position index of the tab. The tab of first position will return 0, the second 1 etc.
...
def IsolateEntitiesInModelBrowser(entities: list[Entity], mode: str='hide') -> bool:

	"""

	Ability to isolate only some entities (MB containers) in the Model Browser and hide or delete all the others.
	If this function is called with mode='hide' then all not related MB containers are just hidden from the Model Browser environment.
	If it is called with mode='delete' then all not related MB containers are totally deleted from the ANSA model. 
	In this case only the isolated entities are kept and all their references.

	Parameters
	----------
	entities : list[Entity]
		The MB Containers (Sim.Models, Sim.Runs, etc.) to be isolated.

	mode : str, optional
		The mode of isolation. 
		The mode='hide' does only hide items, instead the mode='delete' totally deletes all non related items.

	Returns
	-------
	bool
		Returns True if suceeds to isolate the given items else False
	...
	::
		...
		def main():
		    entities = base.CollectEntities(constants.NASTRAN, None, "ANSA_SIMULATION_RUN")
		    if entities is None:
		        print("None Sim Runs!")
		        return
		
		    print(f"Number of existing Sim Runs: {len(entities)}")
		    if len(entities) == 0:
		        print("No Sim Runs!")
		        return
		
		    selected_entities = [entities[0]]
		    print(f"Number of selected Sim Runs: {len(selected_entities)}")
		
		    # success = utils.IsolateEntitiesInModelBrowser(selected_entities)
		    # success = utils.IsolateEntitiesInModelBrowser(selected_entities, mode='delete')
		    success = utils.IsolateEntitiesInModelBrowser(selected_entities, mode="hide")
